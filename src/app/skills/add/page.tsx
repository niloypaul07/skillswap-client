...
import { Plus, Image, AlignLeft, DollarSign, Award, MapPin } from "lucide-react";
...
export default function AddSkillPage() {
  ...
  useEffect(() => {
    if (!loading && !user) {
      toast.error("Please login to add a skill listing.");
      router.push("/login");
    }
  }, [user, loading, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || "Failed to create skill listing.";
      toast.error(msg);
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="flex-1 flex flex-col bg-[#090a0f]">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-12">
        <div className="space-y-2 mb-8 text-center md:text-left">
          <h1 className="text-3xl font-bold text-white tracking-tight">Create a New Skill Listing</h1>
          <p className="text-gray-400">Share your expertise and start tutoring students 1-on-1.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-card-bg border border-white/5 rounded-2xl p-6 md:p-8 space-y-6">
          ...
          >
            {submitting ? <span>Creating Listing...</span> : <span>Publish Skill Listing</span>}
          </button>
        </form>
      </main>
